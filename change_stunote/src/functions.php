...
function itemPatch($shortCode, $TalisGUID, $token, $input, $itemID) {
...
//	var_export($input);
	...
	curl_close($ch);
	if ($info !== 200){
		echo "<p>ERROR: There was an error updating the item:</p><pre>" . var_export($output, true) . "</pre>";
	} else {
...
function etag_fetch_fromItem($shortCode, $itemID, $TalisGUID, $token) {
