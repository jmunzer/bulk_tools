...
function make_resource($shortCode, $title, $resource_type, $isbn, $TalisGUID, $token, $lcn, $full_name, $publisher_name, $web_addresses, $edition) {
...
function itemBody($etag, $listID, $resource_id) {
	$uuid = guidv4();
	$input = '{
				"meta": {
					"list_etag": "' . $etag . '",
...
curl_setopt($ch_alma, CURLOPT_URL, $alma_lookup);
curl_setopt($ch_alma, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch_alma, CURLOPT_HTTPHEADER, array(
    
    'Cache-Control: no-cache'

));

$output_alma = curl_exec($ch_alma);
$info_alma = curl_getinfo($ch_alma, CURLINFO_HTTP_CODE);
curl_close($ch_alma);*/
...
$resource_type = $v->Column1;
$lcn = $v->Column2;
$title = $v->Column3;
$isbn=$v->Column4;
$full_name = $v->Column5;
$publisher_name = $v->Column6;
$web_addresses = $v->Column7;
$edition = $v->Column8;

echo $isbn."</br>";
echo $resource_type."</br>";
echo $lcn."</br>";
echo $title."</br>";
echo $full_name."</br>";
echo $publisher_name."</br>";
echo $web_addresses."</br>";
echo $edition."</br>";
echo "------------</br>";

$resource_id = make_resource($shortCode, $title, $resource_type, $isbn, $TalisGUID, $token, $lcn, $full_name, $publisher_name, $web_addresses, $edition);
$etag = etag_fetch($shortCode, $listID, $TalisGUID, $token);
	$input = itemBody($etag, $listID, $resource_id);
...
//**************DELETE_ITEM*************** 
$patch_url = 'https://rl.talis.com/3/' . $shortCode . '/draft_items/' . $i;
...
if ($info2 !== 200){
  ...
  //*  fwrite($myfile, "Item not deleted - failed" . "\t");
    ...
    echo "    Deleted item $i from list $listID</br>";
...
/*url_setopt($ch_item, CURLOPT_POSTFIELDS, $input);
